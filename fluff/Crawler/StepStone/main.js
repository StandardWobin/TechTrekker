...
(async () => {
    ...
    let start = Date.now();
    await page.goto('https://www.stepstone.de/work/developer/in-berlin?radius=30');
    ...
    let amountQuery = await page.evaluate("document.getElementsByClassName('res-k6x44c at-facets-header-title')[0].innerText")

    // extract the number using regex
    let amount = amountQuery.split(" ")[0];
    ...
    for (let pageIndex = 0; pageIndex < Math.ceil(amount / 25); pageIndex++) {
        if (pageIndex > 0) {
            start = Date.now();
            ...
            await page.goto('https://www.stepstone.de/work/developer/in-berlin?radius=30&page='  + (pageIndex+1));
